...
- **Step 3.4 iOS** — visual QA loop uses XcodeBuildMCP SwiftUI Preview captures (not Playwright screenshots). The loop runs `subagent_type: ios-swift-ui-design` as the generator (Preview tweaks) paired with `subagent_type: design-critic` as the critic (scores against the design board). Exit criterion = user-approved pass/fail (not a 0-100 rubric). **Max 3 iterations** (tighter than web's 5). On stall: present captures + remaining issues to user for accept/reject decision.
...
- **Step 4.0.d (iOS):** Metric Loop on scaffold health — builds clean via XcodeBuildMCP, Swift Testing `@Test`s pass, structure matches architecture. Max 3 iterations. On `xcodebuild` failure, spawn the Swift build resolver — see the dispatch block below.
...
Call the Agent tool — description: "Swift build fix (scaffold)" — subagent_type: `swift-build-resolver` — mode: "bypassPermissions" — prompt: "xcodebuild failed with this error: [paste]. Apply the minimal diff to fix the specific error. No architectural edits, no dependency changes, no refactors. Confirm green before returning."
...
**Agent selection rule for Step 4.1:**
- UI tasks (SwiftUI views, view models, navigation): `subagent_type: ios-swift-ui-design` for the plan step; implementer stays `engineering-senior-developer` or `engineering-mobile-app-builder`.
- Foundation Models / on-device LLM tasks (gated by `ios_features.foundationModels`): `subagent_type: ios-foundation-models-specialist`.
- StoreKit / IAP tasks (gated by `ios_features.storekit`): `subagent_type: ios-storekit-specialist`.
- Exploratory Swift-code search before implementation: `subagent_type: ios-swift-search`.
- General / cross-cutting iOS implementation: `subagent_type: engineering-senior-developer` (L tasks) or `subagent_type: engineering-mobile-app-builder` (S/M mobile-specific tasks).
...
1. Call the Agent tool — description: "Simplify [task name]" — subagent_type: `code-simplifier` — mode: "bypassPermissions" — prompt: "Simplify changed Swift files from [task]. Remove dead code, unused imports, redundant abstractions. Do NOT add features. Do NOT change architecture. Do NOT touch files outside the changeset. Files: [list]."
2. Call the Agent tool — description: "Refactor-clean [task name]" — subagent_type: `refactor-cleaner` — mode: "bypassPermissions" — prompt: "Clean dead code from changed Swift files in [task]. Run SwiftLint / xcodebuild warning sweep and remove orphaned helpers, unused types, dead functions. Same scope rules — changeset only. Files: [list]."
...
Call the Agent tool — description: "Swift build fix" — subagent_type: `swift-build-resolver` — mode: "bypassPermissions" — prompt: "xcodebuild failed with this error: [paste]. Apply the minimal diff to fix the specific error. No architectural edits, no dependency changes, no refactors. Confirm green before returning."
...
Each twin owns its own scope and reviewer — they are NOT merged into a single loop. After all three twins complete, run **Phase 6 Step 6.0 Reality Check** (in `commands/build.md`, backed by `subagent_type: testing-reality-checker`) with iOS evidence (Maestro run logs, Preview captures, HIG findings).
