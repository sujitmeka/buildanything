...
def parse_prose_backward_edges(build_md_path: str) -> list[dict]:
    ...
    for hit in _EDGE_RE.finditer(body):
        fr = hit.group("fr").strip().strip("|").strip()
        to = hit.group("to").strip().strip("|").strip()
...
def _extract_yaml_edges(data: Any) -> list[dict]:
    """Collect backward-routing edges from already-parsed YAML data."""
...
def parse_yaml_backward_edges(yaml_path: str) -> list[dict]:
    ...
    return _extract_yaml_edges(yaml.safe_load(Path(yaml_path).read_text()))
...
def parse_backward_routing(repo: Path) -> list[ParseIssue]:
    ...
    section_match = _SECTION_RE.search(prose_text)
    if section_match is None:
        issues.append(ParseIssue(
            parser=PARSER_NAME, kind="prose_section_missing",
        ...
        if not prose_edges:
            issues.append(ParseIssue(
                parser=PARSER_NAME, kind="prose_edges_empty",
                message=(
                    "backward-routing section header found in build.md but "
                    "body contains no edge rows (regex regression?)"
                ),
                source=PROSE_REL_PATH))
    ...
    yaml_edges = _extract_yaml_edges(yaml_data)
    has_yaml_section = (
    ...
    if not has_yaml_section:
        issues.append(ParseIssue(
            parser=PARSER_NAME, kind="yaml_section_missing",
