...
| 3.3-ios | UX Architecture + Page Layouts (iOS) | dispatch-single | `design-ux-architect` reads product-spec.md (source of truth — Screen Inventory drives screen count), DESIGN.md (Brand DNA + YAML components), architecture.md#frontend, design-references/. Produces `ux-architecture.md` (HIG navigation model, interaction patterns, iPhone+iPad responsive strategy) AND `page-specs/*.md` (one per screen — single mobile-first ASCII wireframe at iPhone 16 Pro 393×852, HIG navigation annotations, DESIGN.md component token refs, Dynamic Type role declarations, safe-area handling). Follows `protocols/page-spec-schema.md` |
| 3.3b-ios | UX Flow Validation (iOS) | dispatch-single | `design-ux-researcher` walks each iOS flow as target persona on an iPhone, checks HIG conformance (gesture discoverability, tap target ≥44pt, navigation depth), checks Dynamic Type at xxxLarge doesn't break layouts; critical issues → backward edge to Step 3.3-ios. Writes `ux-flow-validation.md` |
...
**Phase 4 entry requirement (iOS):** `DESIGN.md` (Pass 1 + Pass 2 complete; broken-refs lint == 0) AND `docs/plans/page-specs/` (at least one file from Step 3.3-ios) must exist.
